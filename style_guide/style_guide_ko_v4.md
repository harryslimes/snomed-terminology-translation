...
- Added **contrast word-order rule** (procedure section): contrast phrase
  appears before the body site (empirical basis: 68:1 in KR procedures).
- **Resolved `절제` vs `절제술` open question** for Excision method:
  default bare `절제` (empirical basis: 525:5 in KR procedures).
- **Enumerated action-suffix discipline per method** based on KR
  frequencies.
- Modality spacing rule strengthened (CT 100% spaced, MRI 99%; do not
  use unspaced forms).
- Updated worked examples to match current KR preference (removed
  `절제술` where the KR reference uses bare `절제`).
...
- **`-술` (operation suffix) discipline** — empirically grounded in v4.
  For the **Excision** method, the bare form `절제` dominates `절제술`
  **525:5** in KR preferred terms. Default to the bare form. Append `-술`
  only for these **fixed compounds** (each confirmed by KR frequency):
    - **`내시경술`** (endoscopy — KR default).
    - **`우회술`** (bypass — 7:1 over bare `우회` in KR).
    - **`절단술`** (amputation — acceptable alongside `절단`).
    - **`창냄술`** (stomy / ostomy creation — 12:0 in KR for
      Construction method; always use `-술` for ostomy creation).
    - **`조성술`, `형성술`, `성형술`** (creation / formation / plastic
      procedures where the source names a named technique).
    - **`이식술`** (transplantation — ~5:2 over bare `이식`; either
      acceptable, prefer `-술` for formal procedure names).
    - **`치환술`, `교환술`** (replacement — split `교환`:`교체`:`치환` as
      5:3:1 for Replacement method; prefer context-appropriate form).
...
#### Contrast word-order (critical)

For imaging procedures that specify contrast use, the KR release is
**near-universal** in placing the contrast phrase **first**, before the
body site. Empirical basis: **68 contrast-first vs 1 site-first** across
all KR imaging procedures with `with/without contrast` in the source
FSN.

**Rule:** `[contrast phrase] [body site] [modality]`, not
`[body site] [contrast phrase] [modality]`.

| English | Correct (contrast-first) | Wrong (site-first) |
|---|---|---|
| Computed tomography of brain without contrast | 조영제 미사용 뇌 컴퓨터 단층 촬영 | 뇌 조영제 미사용 컴퓨터 단층 촬영 |
| Computed tomography of abdomen with contrast | 조영제 사용 복부 컴퓨터 단층 촬영 | 복부 조영제 사용 컴퓨터 단층 촬영 |
| Magnetic resonance imaging of left breast with contrast | 조영제 사용 왼쪽 유방 자기 공명 영상 | 왼쪽 유방 조영제 사용 자기 공명 영상 |

This rule overrides any external guide (e.g. the KHIS radiology
editorial document) that prescribes the opposite ordering.
...
### Open questions (procedure) — remaining
...
- **`절제` vs `절제술`** — resolved: **default bare `절제`** for Excision
  method (525:5 in KR). See the `-술` discipline block above for the
  per-method exception list.
- **Body-site naming (appendix / kidney / colon / etc.)** — resolved:
  default to the Sino-Korean form for viscera, pure-Korean for surface
  anatomy. Specific canonical forms listed in the body-structure section
  above. Noted exceptions where the KR release shows split usage:
  `Bone structure` (뼈 13 vs 골 8, near 50/50),
  `Upper limb structure` (상지 2 vs 팔 2, 50/50),
  `Appendix structure` (충수 8 / 충수돌기 6 / 막창자꼬리 1, 3-way split).

### Worked examples

| SCTID | English PT | Korean reference |
|---|---|---|
| 80146002 | Excision of appendix | 충수 절제 |
...
| 396205005 | Computed tomography of brain without contrast | 조영제 미사용 뇌 컴퓨터 단층 촬영 |
