# Korean SNOMED CT translation style guide (v5.1)

> v3-abbreviated + a narrow set of additions distilled from KHIS clinician
> feedback (May 2026): derivational-suffix preservation, contrast-modifier
> handling, the "symptomatic = diagnostic" radiology idiom, and explicit
> guidance against transliterating unfamiliar `-graphy / -gram` roots.
>
> Changes from v5: the rules that conflicted with the KR-extension's
> conventions (forcing solid imaging compounds like `자기공명영상`,
> forcing native-Korean body sites like `팔` / `다리` everywhere) have
> been **rolled back** — those choices are now framed as acceptable
> variation, not as a target to enforce.
...
below. They fill gaps that the v3 guide left open. They do *not* override
the spacing and body-site defaults — those follow the KR extension as
before.
...
| angiography (vessel-generic) | `혈관 조영(술)` |
...
end of these compounds (e.g. both `정맥 조영상` and `정맥 조영술` appear).
...
**Place the contrast modifier at the FRONT of the term** (before
laterality, site, and modality), not at the end. KR extension precedent
overwhelmingly uses front placement.
...
- **`symptomatic [study]`** in a radiology context is **not** the
  symptom-treatment form. It means *diagnostic* (a study performed
...
- **`screening [study]`** stays as **`선별`** + study
  (e.g. screening mammogram → `선별 유방 영상`). Do not conflate with
  `진단`.
